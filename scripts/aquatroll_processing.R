...
check_headers("C:/Users/samjg/Documents/Github_repositories/FCWC-data-processing/data/csv/VuSitu_2019-10-12_15-16-08_Device Location_LiveReadings.csv")
check_headers("C:/Users/samjg/Documents/Github_repositories/LISS_sonde_data/data/csv/0723ASHC_Sonde.csv")
...
look_aquatroll("C:/Users/samjg/Documents/Github_repositories/FCWC-data-processing/data/csv/VuSitu_2019-10-12_15-16-08_Device Location_LiveReadings.csv")
look_aquatroll("C:/Users/samjg/Documents/Github_repositories/LISS_sonde_data/data/csv/0723ASHC_Sonde.csv")
...
summary_aquatroll("C:/Users/samjg/Documents/Github_repositories/FCWC-data-processing/data/csv/VuSitu_2019-10-12_15-16-08_Device Location_LiveReadings.csv")
summary_aquatroll("C:/Users/samjg/Documents/Github_repositories/LISS_sonde_data/data/csv/0723ASHC_Sonde.csv")
...
)
{
  ...
  H <- as.data.frame(H)
  ### strip serial numbers
  names(D) <- gsub(" \\([0-9]+\\)", "", columns) # ommits the numbers from the column names 
  ### Aquatroll serial number
  row_h <- which(H[,1] == "Device Model = Aqua TROLL 600 Vented")
  aquatroll_sn <- as.numeric(strsplit(as.character(H[row_h+1,1]),'=')[[1]][2]) # grabs the skew number base on relaive position of the prior
  ### Handheld serial number
  row_sn <- grep('Device Model',H[,1])
  row_h2 <- setdiff(row_sn,row_h)
  handheld_sn <- ifelse(length(row_h2)>0,
                        ...
                        NA)
  ### rename temperaures
  ind_aquatroll <- grep(aquatroll_sn,columns) # column numbers that have the skew number 'aquatroll_sn'
  ### Aquatroll temperature
  ind_temp <- grep('Temperature',columns) %>% # call the column number tha thave temperature
    intersect(ind_aquatroll) # and where this interects with the column numbers that have the 'aquatroll_sn'
  names(D)[ind_temp] <- paste(names(D)[ind_temp],'AT') # adds 'AT' to the end of column name containing 'Temperature' 
  ### Handheld temperature
  ind_temp2 <- grep('Temperature',columns) %>%
    setdiff(ind_aquatroll)
  names(D)[ind_temp2] <- paste(names(D)[ind_temp2],'HH')
...
data_extract_aquatroll("C:/Users/samjg/Documents/Github_repositories/FCWC-data-processing/data/csv/VuSitu_2019-10-12_15-16-08_Device Location_LiveReadings.csv")
data_extract_aquatroll("C:/Users/samjg/Documents/Github_repositories/LISS_sonde_data/data/csv/0723ASHC_Sonde.csv")
